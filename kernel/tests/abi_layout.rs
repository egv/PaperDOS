...
use kernel::abi::{
    PdDir, PdDirent, PdFile, PdFont, PdHttp, PdStat, PdSyscallFn, PdSyscalls, PD_ABI_VERSION,
    PD_BTN_BACK, PD_BTN_DOWN, PD_BTN_LEFT, PD_BTN_OK, PD_BTN_POWER, PD_BTN_RIGHT, PD_BTN_UP,
    PD_COLOR_BLACK, PD_COLOR_WHITE, PD_FTYPE_DIR, PD_FTYPE_FILE, PD_LOG_DEBUG, PD_LOG_ERROR,
    PD_LOG_INFO, PD_LOG_WARN, PD_MAX_PATH, PD_REFRESH_FAST, PD_REFRESH_FULL, PD_REFRESH_PARTIAL,
    PD_ROTATION_0, PD_ROTATION_180, PD_ROTATION_270, PD_ROTATION_90, PD_SCREEN_HEIGHT,
    PD_SCREEN_WIDTH, PD_SDK_VERSION, PD_SEEK_CUR, PD_SEEK_END, PD_SEEK_SET, PD_WIFI_CONNECTED,
    PD_WIFI_CONNECTING, PD_WIFI_ERROR, PD_WIFI_OFF,
};

const SYSCALL_SLOT_SIZE: usize = size_of::<PdSyscallFn>();
...
fn syscall_display_block_abi_layout() {
    ...
    assert_eq!(
        ...
        display_base + (SYSCALL_SLOT_SIZE * 1)
    );
    assert_eq!(
        offset_of!(PdSyscalls, display_draw_rect),
        display_base + (SYSCALL_SLOT_SIZE * 2)
    );
    assert_eq!(
        offset_of!(PdSyscalls, display_fill_rect),
        display_base + (SYSCALL_SLOT_SIZE * 3)
    );
    assert_eq!(
        offset_of!(PdSyscalls, display_draw_bitmap),
        display_base + (SYSCALL_SLOT_SIZE * 4)
    );
    assert_eq!(
        offset_of!(PdSyscalls, display_draw_text),
        display_base + (SYSCALL_SLOT_SIZE * 5)
    );
    assert_eq!(
        offset_of!(PdSyscalls, display_refresh),
        display_base + (SYSCALL_SLOT_SIZE * 6)
    );
    assert_eq!(
        offset_of!(PdSyscalls, display_set_rotation),
        display_base + (SYSCALL_SLOT_SIZE * 7)
    );
    assert_eq!(
        offset_of!(PdSyscalls, display_width),
        display_base + (SYSCALL_SLOT_SIZE * 8)
    );
    assert_eq!(
        offset_of!(PdSyscalls, display_height),
        display_base + (SYSCALL_SLOT_SIZE * 9)
    );
}

#[test]
fn syscall_input_block_abi_layout() {
    let input_base = 16 + (SYSCALL_SLOT_SIZE * 10);

    assert_eq!(offset_of!(PdSyscalls, input_get_buttons), input_base);
    assert_eq!(
        offset_of!(PdSyscalls, input_wait_button),
        input_base + (SYSCALL_SLOT_SIZE * 1)
    );
    assert_eq!(
        offset_of!(PdSyscalls, input_get_battery_pct),
        input_base + (SYSCALL_SLOT_SIZE * 2)
    );
}

#[test]
fn syscall_filesystem_block_abi_layout() {
    let filesystem_base = 16 + (SYSCALL_SLOT_SIZE * 13);

    assert_eq!(offset_of!(PdSyscalls, fs_open), filesystem_base);
    assert_eq!(
        offset_of!(PdSyscalls, fs_close),
        filesystem_base + (SYSCALL_SLOT_SIZE * 1)
    );
    assert_eq!(
        offset_of!(PdSyscalls, fs_read),
        filesystem_base + (SYSCALL_SLOT_SIZE * 2)
    );
    assert_eq!(
        offset_of!(PdSyscalls, fs_write),
        filesystem_base + (SYSCALL_SLOT_SIZE * 3)
    );
    assert_eq!(
        offset_of!(PdSyscalls, fs_seek),
        filesystem_base + (SYSCALL_SLOT_SIZE * 4)
    );
    assert_eq!(
        offset_of!(PdSyscalls, fs_tell),
        filesystem_base + (SYSCALL_SLOT_SIZE * 5)
    );
    assert_eq!(
        offset_of!(PdSyscalls, fs_eof),
        filesystem_base + (SYSCALL_SLOT_SIZE * 6)
    );
    assert_eq!(
        offset_of!(PdSyscalls, fs_mkdir),
        filesystem_base + (SYSCALL_SLOT_SIZE * 7)
    );
    assert_eq!(
        offset_of!(PdSyscalls, fs_remove),
        filesystem_base + (SYSCALL_SLOT_SIZE * 8)
    );
    assert_eq!(
        offset_of!(PdSyscalls, fs_opendir),
        filesystem_base + (SYSCALL_SLOT_SIZE * 9)
    );
    assert_eq!(
        offset_of!(PdSyscalls, fs_readdir),
        filesystem_base + (SYSCALL_SLOT_SIZE * 10)
    );
    assert_eq!(
        offset_of!(PdSyscalls, fs_closedir),
        filesystem_base + (SYSCALL_SLOT_SIZE * 11)
    );
    assert_eq!(
        offset_of!(PdSyscalls, fs_stat),
        filesystem_base + (SYSCALL_SLOT_SIZE * 12)
    );
}

#[test]
fn syscall_network_block_abi_layout() {
    let network_base = 16 + (SYSCALL_SLOT_SIZE * 26);

    assert_eq!(offset_of!(PdSyscalls, net_wifi_connect), network_base);
    assert_eq!(
        offset_of!(PdSyscalls, net_wifi_disconnect),
        network_base + (SYSCALL_SLOT_SIZE * 1)
    );
    assert_eq!(
        offset_of!(PdSyscalls, net_wifi_status),
        network_base + (SYSCALL_SLOT_SIZE * 2)
    );
    assert_eq!(
        offset_of!(PdSyscalls, net_http_get),
        network_base + (SYSCALL_SLOT_SIZE * 3)
    );
    assert_eq!(
        offset_of!(PdSyscalls, net_http_post),
        network_base + (SYSCALL_SLOT_SIZE * 4)
    );
    assert_eq!(
        offset_of!(PdSyscalls, net_http_begin),
        network_base + (SYSCALL_SLOT_SIZE * 5)
    );
    assert_eq!(
        offset_of!(PdSyscalls, net_http_set_header),
        network_base + (SYSCALL_SLOT_SIZE * 6)
    );
    assert_eq!(
        offset_of!(PdSyscalls, net_http_send),
        network_base + (SYSCALL_SLOT_SIZE * 7)
    );
    assert_eq!(
        offset_of!(PdSyscalls, net_http_read),
        network_base + (SYSCALL_SLOT_SIZE * 8)
    );
    assert_eq!(
        offset_of!(PdSyscalls, net_http_status_code),
        network_base + (SYSCALL_SLOT_SIZE * 9)
    );
    assert_eq!(
        offset_of!(PdSyscalls, net_http_end),
        network_base + (SYSCALL_SLOT_SIZE * 10)
    );
}

#[test]
fn syscall_system_block_abi_layout() {
    let system_base = 16 + (SYSCALL_SLOT_SIZE * 37);

    assert_eq!(offset_of!(PdSyscalls, sys_sleep_ms), system_base);
    assert_eq!(
        offset_of!(PdSyscalls, sys_millis),
        system_base + (SYSCALL_SLOT_SIZE * 1)
    );
    assert_eq!(
        offset_of!(PdSyscalls, sys_exit),
        system_base + (SYSCALL_SLOT_SIZE * 2)
    );
    assert_eq!(
        offset_of!(PdSyscalls, sys_reboot),
        system_base + (SYSCALL_SLOT_SIZE * 3)
    );
    assert_eq!(
        offset_of!(PdSyscalls, sys_log),
        system_base + (SYSCALL_SLOT_SIZE * 4)
    );
    assert_eq!(
        offset_of!(PdSyscalls, sys_get_free_heap),
        system_base + (SYSCALL_SLOT_SIZE * 5)
    );
    assert_eq!(
        offset_of!(PdSyscalls, sys_wifi_release),
        system_base + (SYSCALL_SLOT_SIZE * 6)
    );
    assert_eq!(
        offset_of!(PdSyscalls, sys_wifi_acquire),
        system_base + (SYSCALL_SLOT_SIZE * 7)
    );
}

#[test]
fn syscall_tail_layout_abi_layout() {
    let memory_base = 16 + (SYSCALL_SLOT_SIZE * 45);
    let font_base = 16 + (SYSCALL_SLOT_SIZE * 48);

    assert_eq!(offset_of!(PdSyscalls, mem_alloc), memory_base);
    assert_eq!(
        offset_of!(PdSyscalls, mem_free),
        memory_base + (SYSCALL_SLOT_SIZE * 1)
    );
    assert_eq!(
        offset_of!(PdSyscalls, mem_realloc),
        memory_base + (SYSCALL_SLOT_SIZE * 2)
    );

    assert_eq!(offset_of!(PdSyscalls, font_load), font_base);
    assert_eq!(
        offset_of!(PdSyscalls, font_free),
        font_base + (SYSCALL_SLOT_SIZE * 1)
    );
    assert_eq!(
        offset_of!(PdSyscalls, font_text_width),
        font_base + (SYSCALL_SLOT_SIZE * 2)
    );
    assert_eq!(
        offset_of!(PdSyscalls, font_line_height),
        font_base + (SYSCALL_SLOT_SIZE * 3)
    );

    assert_eq!(align_of::<PdSyscalls>(), align_of::<PdSyscallFn>());
    assert_eq!(size_of::<PdSyscalls>(), 16 + (SYSCALL_SLOT_SIZE * 52));
}
