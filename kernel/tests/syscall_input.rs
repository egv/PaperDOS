...
fn button_event_to_mask_returns_button_mask_syscall_input() {
    assert_eq!(
        button_event_to_mask(ButtonEvent::Press(ButtonId::Up)),
        PD_BTN_UP
    );
    assert_eq!(
        button_event_to_mask(ButtonEvent::LongPress(ButtonId::Select)),
        PD_BTN_OK
    );
}
...
fn input_get_battery_pct_returns_minus_one_syscall_input() {
    assert_eq!(
        pd_input_get_battery_pct(),
        -1,
        "stub must return -1 (unknown)"
    );
}
...
fn syscall_table_input_fields_populated_syscall_input() {
    ...
    assert_eq!(
        t.input_get_battery_pct,
        pd_input_get_battery_pct as usize as u32
    );
}
